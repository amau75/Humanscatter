...
void setupReceiver(){
    ...
    /* Event queue setup */
    gpio_put(LED, 1);
    //queue_init(&event_queue, sizeof(event_t), EVENT_QUEUE_LENGTH);  //ISSUE HERE
    //queue_init_with_spinlock(&event_queue, sizeof(event_t), EVENT_QUEUE_LENGTH, SPINLOCK);  //ISSUE HERE
    //queue_init_with_spinlock(&event_queue, sizeof(event_t), EVENT_QUEUE_LENGTH, spin_lock_claim_unused(false));  //ISSUE HERE
...
void printPacket(uint8_t *packet, Packet_status status, uint64_t time_us){
    ...
    printf("%02ld:%02ld:%02ld.%03ld | ", hours, minutes, sec, msec);
    if(status.overflowed){
